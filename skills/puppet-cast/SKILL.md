...
2. **Agent teams available?** (`Agent` tool with `name` parameter + `SendMessage` accessible) → Teams mode.
...
- Instanciate a character-agent via the `Agent` tool with `name: {character}`.
...
**Loading contract (per bd-character):** `SKILL.md` frontmatter (name, description) + `writing/` (context, voice, emotions, relations, body) + `core/writing-rules.md` + `snapshots/{current}/encyclopedia/relations/*.md`.

**Required fields:** SKILL.md frontmatter (name, description) + `writing/context.md` + `writing/voice.md` + `core/writing-rules.md`.

**context.md is MANDATORY.** Cumulative narrative arc — the compiled story up to this snapshot. Without it, the puppet knows how the character sounds but not what they lived. If missing, flag it before loading.

**writing/ files are ENGINE, not SCRIPT.** Read them, absorb the rules, then write from inside the character. NEVER parrot, quote, or paraphrase writing/ content in fiction output. The vocabulary of writing/ files has ZERO place in dialogue, narration, or stage directions.
